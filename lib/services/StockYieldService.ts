 ...
 * 
 * 计算美股七巨头在过去 3个月、6个月、1年、3年、5年的收益率
 */
...
export class StockYieldService {
  // 美股七巨头
  private static readonly MAGNIFICENT_SEVEN = [
   ...
   * 识别资产类型（股票）
   */
  private static identifyAssetType(symbol: string): AssetType {
    return 'stock';
  }
  ...
  /**
   * 计算单个股票的收益率
   * 参考 CryptoYieldService 和 ROI Calculator 的实现方式：一次一次调用，不使用并行
   */
  static async calculateYield(symbol: string, name: string): Promise<StockYieldResult | null> {
    const assetType = this.identifyAssetType(symbol);

    console.log(`[StockYieldService] 开始计算 ${symbol} (${name}), 资产类型: ${assetType}`);
    console.log('='.repeat(60));

    try {
      // 1. 获取当前价格（参考 ROI Calculator，使用 CachedPriceService）
      console.log(`[StockYieldService] 步骤1: 获取 ${symbol} 的当前价格...`);
      ...
      if (!currentPriceResult || !currentPriceResult.price || currentPriceResult.price <= 0) {
        console.error(`[StockYieldService] ${symbol} 无法获取当前价格`);
        return null;
      }
      ...
      console.log(`[StockYieldService] ✅ ${symbol} 当前价格: $${currentPrice.toFixed(2)} (来源: ${currentPriceSource})`);
      console.log('');

      // 2. 一次一次获取每个时间段的历史价格（参考 ROI Calculator）
      // 每个请求之间停顿 30 秒，避免触发速率限制
      const timeframes: Record<string, YieldData> = {};

      for (let i = 0; i < this.TIMEFRAMES.length; i++) {
        const tf = this.TIMEFRAMES[i];
        
        // 除了第一个请求，其他请求前都等待 30 秒
        if (i > 0) {
          console.log(`[StockYieldService] ⏳ 等待 30 秒后继续获取 ${symbol} 的下一个时间段数据...`);
        ...
        try {
          ...
          console.log(`[StockYieldService] 步骤${tf.key}: 获取 ${symbol} ${tf.label}前 (${dateStr}) 的历史价格...`);
          ...
          if (
            ...
            console.log(
              `[StockYieldService] ✅ ${symbol} ${tf.label}: 历史价格 $${historicalPrice.toFixed(2)} → 当前价格 $${currentPrice.toFixed(2)} = ${yieldPercent >= 0 ? '+' : ''}${yieldPercent.toFixed(2)}% (来源: ${historicalResult.source})`
            );
          } else {
            console.warn(`[StockYieldService] ⚠️ ${symbol} ${tf.label}: 无法获取历史价格`, historicalResult);
            // 不使用 mock 数据，如果获取失败就标记为不存在
            timeframes[tf.key] = {
              historicalPrice: 0,
              ...
              changeAmount: 0,
              exists: false,
              error: historicalResult?.error || '无法获取历史价格（非 mock 数据）',
            };
          }
        } catch (error: any) {
          console.error(`[StockYieldService] ❌ ${symbol} ${tf.label} 获取失败:`, error.message);
          // 不使用 mock 数据，如果获取失败就标记为不存在
          timeframes[tf.key] = {
            historicalPrice: 0,
            historicalDate: '',
            ...
            changeAmount: 0,
            exists: false,
            error: `获取失败: ${error.message}（非 mock 数据）`,
          };
        }
        console.log('');
      }
      ...
      console.log('='.repeat(60));
      console.log(`[StockYieldService] ✅ ${symbol} 计算完成`);
      ...
      console.error(`[StockYieldService] ❌ ${symbol} 计算失败:`, error.message);
      console.error(`[StockYieldService] 错误堆栈:`, error.stack);
      return null;
    }
  }
   ...
   * 获取美股七巨头列表
   */
  static getMagnificentSeven() {
    return this.MAGNIFICENT_SEVEN;
  }
