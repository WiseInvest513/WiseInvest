/**
 * CryptoYieldService - 加密货币收益率计算服务
 * 
 * 参考 ROI Calculator 的实现方式：
...
export interface CryptoYieldResult {
  symbol: string;
  name: string;
  currentPrice: number;
  currentPriceSource: string;
...
export class CryptoYieldService {
  ...
  /**
   * 计算单个加密货币的收益率
   * 参考 ROI Calculator 的实现方式：一次一次调用，不使用并行
   */
  static async calculateYield(symbol: string, name: string): Promise<CryptoYieldResult | null> {
    const assetType = this.identifyAssetType(symbol);

    console.log(`[CryptoYieldService] 开始计算 ${symbol} (${name}), 资产类型: ${assetType}`);
    console.log('='.repeat(60));

    try {
      // 1. 获取当前价格（参考 ROI Calculator，使用 CachedPriceService）
      console.log(`[CryptoYieldService] 步骤1: 获取 ${symbol} 的当前价格...`);
      ...
      if (!currentPriceResult || !currentPriceResult.price || currentPriceResult.price <= 0) {
        console.error(`[CryptoYieldService] ${symbol} 无法获取当前价格`);
        return null;
      }
      ...
      console.log(`[CryptoYieldService] ✅ ${symbol} 当前价格: $${currentPrice.toFixed(2)} (来源: ${currentPriceSource})`);
      console.log('');
      ...
      for (const tf of this.TIMEFRAMES) {
        try {
          ...
          console.log(`[CryptoYieldService] 步骤${tf.key}: 获取 ${symbol} ${tf.label}前 (${dateStr}) 的历史价格...`);
          ...
          if (
            ...
            console.log(
              `[CryptoYieldService] ✅ ${symbol} ${tf.label}: 历史价格 $${historicalPrice.toFixed(2)} → 当前价格 $${currentPrice.toFixed(2)} = ${yieldPercent >= 0 ? '+' : ''}${yieldPercent.toFixed(2)}% (来源: ${historicalResult.source})`
            );
          } else {
            console.warn(`[CryptoYieldService] ⚠️ ${symbol} ${tf.label}: 无法获取历史价格`, historicalResult);
            timeframes[tf.key] = {
              historicalPrice: 0,
          ...
          console.error(`[CryptoYieldService] ❌ ${symbol} ${tf.label} 获取失败:`, error.message);
          timeframes[tf.key] = {
            historicalPrice: 0,
            historicalDate: '',
            historicalSource: 'Error',
      ...
      console.log('='.repeat(60));
      console.log(`[CryptoYieldService] ✅ ${symbol} 计算完成`);
      ...
      console.error(`[CryptoYieldService] ❌ ${symbol} 计算失败:`, error.message);
      console.error(`[CryptoYieldService] 错误堆栈:`, error.stack);
      return null;
    }
  }
